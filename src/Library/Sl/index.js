...
export default () => {
	useEffect(() => {
		const { slider, items, prev, next } = refs;
		console.log("refs", refs);
		slide(slider, items, prev, next);
	}, []);

	return (
		<div
			id="slider"
			ref={item => createRef(item, "slider")}
			className="slider">
			<div className="wrapper">
				<div
					id="items"
					className="items"
					ref={item => createRef(item, "items")}>
			...
			<a
				...
				ref={item => createRef(item, "prev")}></a>

			<a
				id="next"
				className="control next"
				ref={item => createRef(item, "next")}></a>
		</div>
	);
};
